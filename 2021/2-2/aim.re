...
let set_pos = (ins: instruction, pos: position) : position => {
  let (old_f, old_d, old_a) = pos
  switch(ins){
    | ("forward", num) => (old_f+num, old_d + (old_a*num), old_a)
    | ("down", num) => (old_f, old_d, old_a+num)
    | ("up", num) => (old_f, old_d, old_a-num)
    | _ => pos
  }
}
